...
public final class AnchorTrap extends Module
{
    ...
    public AnchorTrap()
    {
        super("AnchorTrap", new String[]
                { "AnchorTrap" }, "Traps enemies in END PORTALS", "NONE", 0x24DB43, ModuleType.COMBAT);
    }
    ...
    public void onEnable()
    {
        ...
        {
            SalHack.SendMessage(String.format("[AutoTrap] You do not have any %s in your hotbar!", ChatFormatting.BLUE + (EChests.getValue() ? "Ender Chests" : "Obsidian") + ChatFormatting.RESET));
            toggle();
        }
    }
    ...
    public void onDisable()
    {
        ...
        if (announceUsage.getValue())
            SalHack.SendMessage("[AnchorTrap] Disabled!");
    }
    ...
    private Listener<EventPlayerMotionUpdate> OnPlayerUpdate = new Listener<>(p_Event ->
    {
        ...
        if (closestTarget == null)
        {
            if (firstRun)
            {
                ...
                if (announceUsage.getValue())
                {
                    SalHack.SendMessage("[AnchorTrap] Enabled, waiting for target.");
                }
            }
            return;
        }
        if (firstRun)
        {
            ...
            if (announceUsage.getValue())
            {
                SalHack.SendMessage("[AnchorTrap] Enabled, target: " + lastTickTargetName);
            }
        }
        else if (!lastTickTargetName.equals(closestTarget.getName()))
        {
            ...
            if (announceUsage.getValue())
            {
                SalHack.SendMessage("[AnchorTrap] New target: " + lastTickTargetName);
            }
        }
