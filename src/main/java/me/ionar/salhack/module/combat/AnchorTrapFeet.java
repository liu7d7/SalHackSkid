...
public class AnchorTrapFeet extends Module
{
    ...
    public AnchorTrapFeet()
    {
        super("AnchorTrapFeet", new String[]
                { "AnchorTrapFeet" }, "Traps enemies with holes at their feet with END PORTAlS", "NONE", 0x24DB78, ModuleType.COMBAT);
    }
    ...
    public void onEnable()
    {
        ...
        if (Target != null)
        {
            SalHack.SendMessage("[AnchorTrapFeet]: Found target " + Target.getName());
        }
    }
    ...
    private Listener<EventPlayerMotionUpdate> OnPlayerUpdate = new Listener<>(p_Event ->
    {
        ...
        if (IsCurrentTargetTrapped())
        {
            ...
            {
                toggle();
                SalHack.SendMessage(ChatFormatting.LIGHT_PURPLE + "[AnchorTrapFeet]: Current target is trapped. Toggling");
            }
            return;
        }

        if (!HasEndPortalFrame())
            return;
        ...
        int lastSlot;
        final int slot = findStackHotbar(Blocks.END_PORTAL_FRAME);
        if (hasStack(Blocks.END_PORTAL_FRAME) || slot != -1)
        {
            if ((mc.player.onGround && playerSpeed <= 0.005f))
            {
                ...
                if (!slotEqualsBlock(lastSlot, Blocks.END_PORTAL_FRAME))
                {
    ...
    public boolean HasEndPortalFrame()
    {
        return findStackHotbar(Blocks.END_PORTAL_FRAME) != -1;
    }
}
